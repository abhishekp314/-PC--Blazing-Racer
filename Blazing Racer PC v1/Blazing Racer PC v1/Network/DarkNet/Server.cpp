...
void Server::_ReadNetworkData()
{
	...
	while(bytes_recv != -1 )
	{
		...
		DarkNet::XNAddrToInAddr(header.m_xnaddr, m_xnetInfo.m_xnkid, addr);
#endif
		...
		//clear buffer you geeth, asshole!
		/*strcpy(buffer,"");*/
	}  
...
#ifdef _XBOX
XNetInfo Server::GetXnetInfo()
#else
char* Server::GetXnetInfo()
#endif
{
#if defined(_XBOX)
	//SocketAddress addr = DarkNet::GetSocketAddress(m_xnetInfo.m_xnaddr, m_xnetInfo.m_xnkid);
	return m_xnetInfo;
	//return DarkNet::GetIp(&addr);
#elif (WIN32)
	return DarkNet::GetIp(&m_serverAddr);		//this is INADDR_ANY, so useless :(
#endif
